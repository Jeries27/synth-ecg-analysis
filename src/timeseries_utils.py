__all__ = ['butter_filter', 'butter_filter_frequency_response', 'apply_butter_filter', 'save_dataset', 'load_dataset',
           ...
           'npys_to_memmap_batched', 'npys_to_memmap', 'reformat_as_memmap', 'ECGDataset', 'TimeseriesDatasetCrops', 'RandomCrop',
...
class ECGDataset(Dataset):
    def __init__(
        ...
        """
        A Dataset loader class to extract training data for time series forecasting.

        df (DataFrame):                 shape: (N, 46) includes 46 attributes for each patient such as: id, age, labels
        memmap_filename (str):          path to memmap file
        transforms (torch):             the dataset assumes toTensor()
        """
        # assertions
        assert task in ['age', 'generate', 'forecast'] and mmap_filename is not None

        # init
        self.seq_len = 1000 if task == "generate" else seq_len
        self.pred_len = 0 if task == "generate" else pred_len
        self.task = task
        self.B = B
        # data init
        self.mmap_meta_filename = mmap_filename.parent / (mmap_filename.stem + "_meta.npz")
        mmap_meta = np.load(self.mmap_meta_filename, allow_pickle=True)
        self.mmap_start = mmap_meta["start"] # [0 1000 2000 3000 4000 ... 21799000]
        self.mmap_shape = mmap_meta["shape"]
        self.mmap_dtype = np.dtype(str(mmap_meta["dtype"]))
        self.mmap_filenames = np.array(mmap_meta["filenames"]).astype(np.string_)  # save as byte to avoid issue with mp

        self.timeseries_df_data = np.array(df["data"]) # [N, Classes]
        ...
        self.c = len(self.timeseries_df_label[-1])
    ...
    def __getitem__(self, index):
        idx = self.mmap_start[self.timeseries_df_data[index]]
        mem_filename = str(self.mmap_filenames[0], encoding='utf-8')
        mem_file = np.memmap(self.mmap_meta_filename.parent / mem_filename, self.mmap_dtype, mode='r',
                             shape=tuple(self.mmap_shape[0]))

        if self.task == 'age':
            assert self.age is not None
            seq = np.copy(mem_file[idx:idx+(self.seq_len+self.pred_len)])
            del mem_file
            return seq, self.age
        ...
        seq_x = torch.tensor(mem_file[s_begin:s_end]) # (self.seq_len, 12)
        seq_y = torch.tensor(mem_file[r_begin:r_end])
        del mem_file
        label = torch.tensor(self.timeseries_df_label[index])

        # Batch size is 1 by default
        seq_x = torch.unsqueeze(seq_x, 0).view((self.B,12,self.seq_len))
        seq_y = torch.unsqueeze(seq_y, 0).view((self.B,12,self.pred_len))
        label = torch.unsqueeze(label, 0).view((self.B,self.c))

        return seq_x, seq_y, label
